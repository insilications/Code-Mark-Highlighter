...
const filterTagEl: HTMLSelectElement = document.getElementById("filter-tag")! as HTMLSelectElement;
 ...
 *
 * 1. If `tagQuery` is non-empty, the highlight's `tagSearch` must contain the normalized tag query. If
 *    `tagQuery` is empty, no tag filtering is applied.
...
function render(): void {
  ...
  const totalHighlights = filteredFileHighlights.reduce((acc, fh) => acc + fh.highlights.length, 0);
